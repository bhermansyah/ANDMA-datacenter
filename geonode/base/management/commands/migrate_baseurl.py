...
from geonode.utils_signals import designals, resignals
...
Styles and Links Base URLs from [%s] to [%s]." % (source_address, target_address)
        ...
        if force_exec or confirm(prompt=message, resp=False):
            try:
                ...
                for document in documents:
                    print "Checking Document[%s]" % (document)
                    if document.thumbnail_url:
                        original = document.thumbnail_url
                        document.thumbnail_url = document.thumbnail_url.replace(source_address, target_address)
                        document.save()
                        print "Updated Thumbnail URL from [%s] to [%s]" % (original, document.thumbnail_url)
