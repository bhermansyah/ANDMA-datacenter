...
all_data = MapLayer.objects.filter(local=True)

for i in all_data:
	st = json.loads(i.layer_params)
	# layer = Layer.objects.filter(typename=i.name)
	# layer = get_object_or_404(Layer, **{'typename': i.name})
	# print layer.attribute_config()
	# print 'versus \n'
	# print st
	# print 'End \n'
	# print layer.abstract
	# st['capability']['abstract'] = layer.abstract
	if 'capability' in st:
